...
export function IdentityShareCard({ nodes }: IdentityShareCardProps) {
  ...
  // Pick top 5 highest-confidence nodes for the card
  ...
  async function handleShare() {
    ...
    try {
      ...
      setShareUrl(window.location.origin + '/identity/preview')
      setShared(true)
    } finally {
      setLoading(false)
    }
  ...
  async function handleNativeShare() {
    ...
    if (navigator.share) {
      await navigator.share({
        ...
        text: 'Here is who I am, according to my AI memory layer.',
        url: shareUrl,
      })
    } else {
    ...
    <div className="rounded-xl bg-[#141620] border border-white/5 overflow-hidden">
      ...
      <div className="p-5 space-y-4 bg-gradient-to-br from-[#141620] to-[#0D0E18]">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-medium text-[#7B6CF6] uppercase tracking-widest">ECHO Identity</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {topNodes.map(node => (
            <span
              ...
              style={{
                color: TYPE_COLORS[node.type] ?? '#8B8FA8',
                borderColor: `${TYPE_COLORS[node.type] ?? '#8B8FA8'}30`,
                backgroundColor: `${TYPE_COLORS[node.type] ?? '#8B8FA8'}10`,
              }}
            >
              {node.label}
            </span>
        ...
        <p className="text-[10px] text-[#8B8FA8]">
          {nodes.length} identity signals detected · echo-self.app
        </p>
      </div>
