...
export default function WrappedPage() {
  ...
  return (
    <main className="min-h-screen max-w-xl mx-auto px-4 py-12 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {data.stats.topIdentityTraits.map(trait => (
              <span key={trait} className="text-xs px-3 py-1 rounded-full bg-[#7B6CF6]/10 border border-[#7B6CF6]/20 text-[#7B6CF6]">
                {trait}
              </span>
            ))}
          </div>
      ...
      <button onClick={() => setData(null)} className="w-full text-xs text-[#8B8FA8] hover:text-[#F0F0F5] transition-colors">
        Regenerate
      </button>
    </main>
  )
