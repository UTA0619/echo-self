...
export async function POST() {
  ...
  // Gather year's data in parallel
  const [entriesRes, emotionsRes, identityRes, patternsRes] = await Promise.all([
    supabase
      .from('entries')
      .select('created_at, emotion, emotion_score, content')
      .eq('user_id', user.id)
      .gte('created_at', yearStart)
      .order('created_at', { ascending: true }),
    supabase
      .from('entries')
      .select('emotion, emotion_score')
      .eq('user_id', user.id)
      .gte('created_at', yearStart)
      .not('emotion', 'is', null),
    supabase
  ...
  if (entries.length < 5) {
    return NextResponse.json({
      error: 'Not enough entries for a Wrapped. Keep journaling!',
  ...
  // Sample entries for Claude
  const sampleEntries = entries
    .filter((_, i) => i % Math.max(1, Math.floor(entries.length / 8)) === 0)
    .slice(0, 8)
...
- ${entryCount} journal entries written
...
## Task
Write a beautiful, personal "ECHO Wrapped" summary for their year. Structure it as:
1. A one-paragraph narrative of their year's emotional arc (warm, poetic, 3-4 sentences)
2. Their "word of the year" — a single word that captures their growth
3. A one-sentence "growth headline" (e.g. "The year you learned to trust yourself")
4. A forward-looking sentence about next year
...
Return ONLY valid JSON:
{
  ...
  return NextResponse.json({
    ...
    stats: {
      entryCount,
      dominantEmotion,
      avgEmotionalScore: Math.round(avgEmotionalScore * 100) / 100,
      topIdentityTraits: identityNodes.map(n => n.label),
    },
    ...wrapped,
  })
}
